...
function RoleRow({ profile, action }: { profile: Profile; action: Action }) {
  ...
  return <tr><td><b>{profile.full_name || "İsimsiz kullanıcı"}</b><div className="sub mono">{profile.id.slice(0, 8)}…</div></td><td><form action={formAction} className="role-form"><input type="hidden" name="id" value={profile.id} /><select className="input" name="role" defaultValue={profile.role}><option value="sales">Satış Personeli</option><option value="admin">Admin</option><option value="super_admin">Süper Admin</option></select><input className="input" name="current_password" type="password" placeholder="Mevcut şifre" required /><button className="btn btn-ghost btn-sm" type="submit" disabled={pending}>{pending ? "…" : "Kaydet"}</button></form>{state && <span className={state.ok ? "form-success" : "form-error"}>{state.message}</span>}</td><td><span className="pill">{profile.active ? "Aktif" : "Pasif"}</span></td><td className="sub">{new Date(profile.created_at).toLocaleDateString("tr-TR")}</td></tr>;
}
